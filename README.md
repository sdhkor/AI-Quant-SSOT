# AI-Quant-SSOT

Public canonical docs for AI Quant operations.

## Canonical Docs

### 1) HQ Dashboard
RAW:
https://raw.githubusercontent.com/sdhkor/AI-Quant-SSOT/refs/heads/main/HQ_DASHBOARD.md

BROWSER:
https://github.com/sdhkor/AI-Quant-SSOT/blob/main/HQ_DASHBOARD.md

### 2) Roles Fixed
RAW:
https://raw.githubusercontent.com/sdhkor/AI-Quant-SSOT/refs/heads/main/ROLES_FIXED_v1_0.md

BROWSER:
https://github.com/sdhkor/AI-Quant-SSOT/blob/main/ROLES_FIXED_v1_0.md

### 3) Work Order
RAW:
https://raw.githubusercontent.com/sdhkor/AI-Quant-SSOT/refs/heads/main/WORK_ORDER_260316.md

BROWSER:
https://github.com/sdhkor/AI-Quant-SSOT/blob/main/WORK_ORDER_260316.md

### 4) Comment Rules
RAW:
https://raw.githubusercontent.com/sdhkor/AI-Quant-SSOT/refs/heads/main/COMMENT_RULES_v1_0.md

BROWSER:
https://github.com/sdhkor/AI-Quant-SSOT/blob/main/COMMENT_RULES_v1_0.md

## Operating Rule
- Canonical docs = public raw `.md`
- Issue/comment = discussion only
- Public에는 민감정보/전략 상세/원본 로그/Private 링크를 기록하지 않음
